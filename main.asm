...
; Created  : 2019-05-20 23:12 UTC
; Modified : 2019-06-09	22:45 UTC
...
.equ T_ZRO = HIGH(150000/PPD)	; High byte of 181mS ($0b0c) for comparison to TC1
.equ T_ONE = HIGH(450000/PPD)	; High byte of 181mS ($1d4c) for comparison to TC1
.equ T_MRK = HIGH(750000/PPD)	; High byte of 181mS ($2f9b) for comparison to TC1
.equ T_MRKH = HIGH(950000/PPD)	; High byte of 181mS ($320c) for comparison to TC1
...
; Various Flags in WWVB Time Code
.equ LY = 3
.equ LS = 2
.equ DST = 0
.equ MINUS = 1
...
; Display Setup - Decode Mode
	...
	ldi	display2L, $37			; All but left-most digit Code B decode
...
; Increment Seconds
	...
	sbrs	mark_num, 6			; If 6th marker received
	rjmp	Inc_Secs			; If not 6th marker, goto Inc_Secs; else:
...
; Increment Days
	...
	cpi	temp, $60			; Compare 1s, look for end of common year
	sbrc	databyte6, LY			; Skip next (leap year) compare if leap year flag not present
	cpi	temp, $70			; Compare 1s, look for end of leap year
...
; Display Seconds
	...
	cbr	display1L, 1<<7			; Clear 7th bit in register to prevent lighting decimal pt
	rcall	Display_Write			; Write changes to display via SPI
...
; Display Days (100s & 10s)
	...
	cbr	display2L, 1<<7			; Clear 7th bit in register to prevent lighting decimal pt
	rcall	Display_Write			; Write changes to display via SPI

; Display DUT1 Sign
	ldi	display1H, D_DIG2		; Display 1, digit 2
	ldi	display1L, $80			; Display 1, decimal point only
	sbrc	databyte4, MINUS		; Check for minus bit
...
; Display Year (10s)
	ldi	display1H, D_NOP		; Display 1, NOP
	ldi	display1L, D_NOP		; Display 1, NOP
	ldi	display2H, D_DIG6		; Display 2, digit 6
...
; Display LY/LS/DST
	ldi	display1H, D_NOP		; Display 1, NOP
	ldi	display1L, D_NOP		; Display 1, NOP
	ldi	display2H, D_DIG7		; Display 2, digit 7
	ldi	display2L, $00			; Display 2, clear
	sbrc	databyte6, DST			; Check for DST
	sbr	display2L, 1<<6			; If not clear, turn on segment A for DST
	sbrc	databyte6, LS			; Check for LS
	sbr	display2L, 1<<3			; If not clear, turn on segment D for LS
	sbrc	databyte6, LY			; Check for LY
	sbr	display2L, 1<<0			; If not clear, turn on segment G for LY
	rcall	Display_Write			; Write changes to display via SPI
...
; Display Blank Digit 4 on Display 2
	ldi	display1H, D_NOP		; Display 1, NOP
	ldi	display1L, D_NOP		; Display 1, NOP
	ldi	display2H, D_DIG4		; Display 2, digit 4
	ldi	temp, D_NOP			; Blank digit
	mov	display2L, temp			; Display 2, digit 4 blank
	rcall	Display_Write			; Write changes to display via SPI
...
; Start decoding bits
	inc	bit_num				; Increment bit number
...
Set_Frame_Bit:	
	ori	bit_val, 1<<FRM			; Set FRM in bit_val
	ldi	mark_num, 1			; Reset marker bit position at start of minute
	clr	bit_num				; Clear bit number register
	clr	data_rx				; Clear data after each marker
	clr	databyte0			; Clear seconds again for leap second
